...
export function ascend(
  ...
  const moons = new Map<string, number>([
    ...
    ["blender", 8],
    ["packrat", 9],
    ["knoll", myPrimestat() === $stat`muscle` ? 1 : myPrimestat() === $stat`mysticality` ? 2 : 3],
    ["canada", myPrimestat() === $stat`muscle` ? 4 : myPrimestat() === $stat`mysticality` ? 5 : 6],
    ["gnomads", myPrimestat() === $stat`muscle` ? 7 : myPrimestat() === $stat`mysticality` ? 8 : 9],
  ]);
